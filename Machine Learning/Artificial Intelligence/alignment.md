...
![[reward.png]]
The goal is to learn the reward model $r_\theta$. We use the **Bradley-Terry** probabilistic model as the basis for our learning objective. For two responses $\tau_i \succ \tau_j$ to a prompt $s$, where response $i$ is preferred to $j$, we want the probability of seeing $\tau_i$ to be exponentially higher as the difference in reward increases:
$$p(\tau_i \succ \tau_j) = \log \sigma(r_\theta(s, \tau_i) - r_\theta(s, \tau_j))$$
where $\sigma$ is the [[logistic regression|logistic]] function. We can then learn the parameters $\theta$ for our reward model under the objective:
$$\theta \leftarrow \arg\max_\theta \sum_{i, j} \log \sigma (r_\theta(\tau_i) - r_\theta(\tau_j))$$
## fine-tuning with reinforcement learning
Fine-tuning with RL is a bit different from traditional fine-tuning, because we must re-frame the fine-tuning task as an RL problem. Here we will use the **contextual bandits** framework, which formalizes fine-tuning with RL as a one-step deterministic [[markov decision processes|MDP]].
...
- **Action space**: the action space of the policy is all the unique tokens in the LM's vocabulary. The actual action taken $a$ is the model response to an input prompt $s$.
...
- **Reward function**: combination of the reward model $r(s,a)$ and a constraint on policy shift

The actual update process is performed by passing in a prompt $s$ and its output $a$ generated by the current iteration of the fine-tuned policy into the trained reward model. The RM returns $r_{\theta}$, a scalar measure of the "preferability" of this prompt-generation pair.
### trust region methods
We then impose a penalty $r_{KL}$ to prevent the RL policy's per-token probability distribution from diverging too far from the initial distribution in the pre-trained model. For our updated policy under parameters $\theta'$ and original policy under $\theta$, we enforce
$$\sum_a |\pi_{\theta'}(a_t|s_t) - \pi_{\theta}(a_t|s_t)| \le \epsilon$$


This is done by computing the [[information theory#kullback-leibler divergence|Kullback-Leibler (KL) divergence]] between the distributions and scaling by a $\lambda$ factor. 
...
- The relative performance is computed by subtracting an average reward baseline from the reward term in the [[policy gradient]]:$$b(s) = \frac 1 K \sum_{k=1}^{K} r(s, a_k)$$where each $a_k$ is a generated response to prompt $s$.
